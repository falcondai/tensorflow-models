    ...
    for image, action in zip(images[:-1], actions[:-1]):
        ...
        with slim.arg_scope(
            ...
            if dna:
                ...
                # XXX another layer with special kernel (static)
            ...
            # the first (additional) mask is background
            ...
            for layer, mask in zip(transformed, mask_list[1:]):
                output += layer * mask
                gen_tf_layers.append(layer)
                gen_masks.append(mask)
            gen_kernels.append(kernels)
